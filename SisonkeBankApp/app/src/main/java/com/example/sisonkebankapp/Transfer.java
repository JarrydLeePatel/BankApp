...
public class Transfer extends MainActivity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extras != null) {
            email = Objects.requireNonNull(Objects.requireNonNull
                    (extras.getExtras())).getString("email");
    ...
    public void transferAmount(View view) {
        if(hasBlankInputs()){
            ...
            double sum2;
            if(!type){
                if(userinfo.getCURRENT_BALANCE() >
                        Double.parseDouble(amount.getText().toString())) {
                    sum1 = userinfo.getCURRENT_BALANCE()
                            - Double.parseDouble(amount.getText().toString());
                    sum2 = userinfo.getSAVINGS_BALANCE()
                            + Double.parseDouble(amount.getText().toString());
                    userinfo.setCURRENT_BALANCE(sum1);
                    userinfo.setSAVINGS_BALANCE(sum2);
                    int i = database.updateBalance(userinfo);
                    if (i == 1){
                        Toast.makeText(this,"Successful transfer",
                                Toast.LENGTH_SHORT).show();
                        this.current.setText(format("%.2f",userinfo.getCURRENT_BALANCE()));
                        this.savings.setText(format("%.2f",userinfo.getSAVINGS_BALANCE()));
                    }
                } else {
                    Toast.makeText(this,"You have insufficient Current Account funds",
                            Toast.LENGTH_SHORT).show();
                }
            } else {
                if(userinfo.getSAVINGS_BALANCE() > Double.parseDouble(amount.getText().toString()))
                {
                    sum1 = userinfo.getSAVINGS_BALANCE()
                            - Double.parseDouble(amount.getText().toString());
                    sum2 = userinfo.getCURRENT_BALANCE()
                            + Double.parseDouble(amount.getText().toString());
                    userinfo.setCURRENT_BALANCE(sum2);
                    userinfo.setSAVINGS_BALANCE(sum1);
                    int i = database.updateBalance(userinfo);
                    if (i == 1){
                        Toast.makeText(this,"Transfer successful.",
                                Toast.LENGTH_SHORT).show();
                        this.current.setText(format("%.2f",userinfo.getCURRENT_BALANCE()));
                        this.savings.setText(format("%.2f",userinfo.getSAVINGS_BALANCE()));
                    }
                } else {
                    Toast.makeText(this,"You have insufficient savings Account funds",
                            Toast.LENGTH_SHORT).show();
                }
            }
        }
